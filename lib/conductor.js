...
exports.cleanConductorStorage = (baseDir) => {
    rimraf.sync(Config.chainStorageDir(baseDir));
    rimraf.sync(Config.uiStorageDir(baseDir));
};
exports.initializeConductorConfig = (baseDir, keyData) => {
...
exports.keygen = (bundlePath) => {
    ...
    const output = child_process_1.execSync(`hc keygen --quiet ${extraArgs}`, {
...
exports.spawnConductor = (baseDir) => {
    console.log("Using conductor binary: ", child_process_1.execSync('which holochain').toString());
    const conductor = child_process_1.spawn('holochain', ['-c', baseDir]);
