...
exports.default = (masterClient, publicClient, internalClient) => (call) => __awaiter(this, void 0, void 0, function* () {
    ...
    const { agentId, happId, zome: zomeName, function: funcName, } = call;
    ...
    const dna = yield install_happ_1.lookupDnaByHandle(masterClient, happId, handle);
    const dnaHash = dna.hash;
    // see if this instance is actually hosted, we may have to get the host's instance if not
    const instance = yield common_1.lookupHoloInstance(publicClient, { agentId, dnaHash });
    // use the looked-up instance info, not the info passed in to the zome call
    const instanceId = common_1.instanceIdFromAgentAndDna(instance);
    const requestData = exports.buildServiceLoggerRequestPackage({
        dnaHash,
        zome: zomeName,
        function: funcName,
        args,
    ...
    const result = yield common_1.zomeCallByInstance(publicClient, {
...
exports.buildServiceLoggerRequestPackage = ({ dnaHash, zome, function: func, args }) => {
    return {
